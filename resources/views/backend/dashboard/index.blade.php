	...
	<div class="row">
		...
		<div class="col-9">
			<select name="tahun" id="tahun" class="form-control" required>
				...
				@for ($i = 0; $i < 5; $i++)
					<option value="{{date('Y')-$i}}" {{request()->get('tahun') == date('Y')-$i ? 'selected' : ''}}>{{((int)date('Y'))-$i}}</option>
				@endfor
			</select>
		</div>
		<div class="col-2">
			<button type="submit" class="btn btn-sm btn-primary">Filter</button>
			<a href="{{route('backend.dashboard.index')}}" class="btn btn-sm btn-danger ">Reset</a>
		</div>
	</div>
</form>
<div class="row">
        ...
        "collapsed": true,
			...
			.then(pins => {
				...
				pins.forEach(function(pin) {
					...
					L.marker(pin.location, { icon: icon })
						.addTo(map)
						.bindPopup(pin.name);
				});
			})
